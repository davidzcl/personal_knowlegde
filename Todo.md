...
*为什么需要记忆*：“让没有记忆的LLM表现出像有连续人格的角色”的最小工程方案。

Memory stream：先把所有原始经历留下，需要的时候再加工。
reflection：定期回顾最近的记忆，生成一些更抽象的判断。从“经历”中提炼“知识”。
retrieve：从 Memory stream 中挑出最相关的若干记忆塞进 prompt。记忆权重公式=recently（越近越相关）+importance（越重要越相关）+similarity（语义越近越相关）

*记忆是什么*

1、工作记忆：ai 在当前一轮对话中能看见的东西，包括 system prompt、当前对话历史、刚检索到的资料、刚调用工具的结果。这类数据组成 LLM 的 context window，对应短期记忆。
2、语义记忆：关于“事实”的记忆，这类记忆是去掉时间和场景的、稳定的，可以反复使用的知识。对应人类的“我知道某件事”，但不一定记得是什么时候、在哪里学到的。
3、情境记忆：关于“经历”的记忆，这类记忆是带时间戳、带场景、像故事一样的。对应人类的“我记得那天我在哪里、和谁、说了什么”。
4、程序记忆：关于“怎么做事”的记忆。包括 agent 的系统提示词、行为规则、惯用的工作流、调用工具的偏好模式。对应人类的“我会骑自行车、我会打字”，属于本能行为。

不同类型的记忆在工程上要用完全不同的方式处理：
- 语义记忆适合存进向量库，按相似度检索；
- 情境记忆需要按时间索引，最好带“什么时候发生”的元数据；
- 程序记忆不能和事实混在一起，否则 agent 会被噪音干扰；
- 工作记忆要主动管理大小，太多东西会导致 LLM 注意力下降

*怎么做记忆*

LLM 的上下文窗口有限，记忆再多也塞不进去。让 LLM 像人用电脑一样使用记忆——不重要的东西先放在硬盘，需要的时候再调到桌面上。
“什么留在眼前、什么暂存到外部、什么时候再调回来”——相当于 ai 拥有无限记忆。

1、核心记忆：存最重要的几件事：用户是谁、agent 自己是谁、当前任务的核心目标。关键设计是 agent 可以自己改写内容。（程序记忆+语义记忆的关键部分）
2、对话记忆：agent 和用户的所有历史对话档案。按时间排序，agent想找哪段聊天可以主动搜索。（情境记忆）
3、资料库：长期外部知识库。文档、笔记、用户上传的资料等，agent 可以主动搜索。（语义记忆）

“内存压力管理”：当对话越来越长、LLM 的工作记忆快被填满时，系统发送压缩管理信号，agent 自主决定：哪些聊天内容可以扔掉、哪些应该转存到对话记忆、哪些值得提炼成事实写入核心记忆。
...
混合检索，所带来的不同检索方式得到不同的分数的问题，通过倒数排名融合解决。

RRF工作原理：
- 对于每个检索方法返回的结果排序
- 查看某个特定文档在每个列表中的排名
- 为每个文档计算“融合分数”，取排名的倒数
- 为避免排名靠后的文档导致影响力过小，给予所有参与排名的文档一个基础权重
- 将一个文档在所有不同列表中的‘倒数排名分数’加起来，得到最终的RRF分数
- 根据所有文档的RRF总分重新排序，得到最终的混合检索结果

不关心原始分数是多少，只关心一个文档在各个列表中的‘排名位置’。
...
- agent.md / README：这是什么系统？
- ARCHITECTURE.md / 模块文档：怎么组织的？
- MakeFile / init.sh / package scripts：怎么跑？
- 测试、lint、check命令：怎么验证？
- PROGRESS.md / 功能清单 / git历史：现在做到哪了？
...
知识点靠近代码：一条关于api端点认证的规则，应该放在api代码旁边。每个模块目录下放一个简短的文档，描述该模块的职责、接口和特殊约束。
最小但完备：每条知识都应该有明确的使用场景。如果删除某条规则不影响agent的决策质量，那这条规则就不应该存在。
和代码一起更新：把架构文档放在对应的模块目录里。改代码的时候自然会看到文档，让CI提醒文档是否需要更新。

原子性：每次‘逻辑操作’用一个git commit原子化；
一致性：定义‘一致状态’的验证——所有测试通过、lint无报错；
隔离性：多个agent并发工作时，状态文件要避免竞争条件。每个agent用独立的进度文件，或者用git分支隔离；
持久性：关键的项目知识用git跟踪的文件持久化。临时状态可以只在会话内存中，但跨会话必须的知识必须写到文件里。
...
python函数，接收当前 state，返回更新后的 state （部分字段）。可以在函数中再定义函数，外层调用时返回 Callable（返回一个可调用对象）

```python
def create_xxx_node() -> Callabel[[内部函数接收的参数], Coroutine[Any, Any, 协程的最终返回值类型]]:
	async def xxx(参数)->返回值类型:
```

## Edge

普通边：固定路径，add_edge("xxx", "yyy")；
条件边：根据 state 动态路由，add_conditional_edges("节点名称", 条件函数, [分发节点名称（可选）])；
起始边：START -> 第一个节点；结束边：某节点 -> END
...
inputstate：定义的入口契约，用于外部调用 graph.ainvoke(input) 时传入的数据。
overallstate：全局状态，工作流执行期间所有节点读写的共享状态。
outputstate：出口契约，工作流结束后返回给调用者的数据。

字段的更新，需要基于节点返回的字典键。避免在节点中直接修改传入的 state，而是返回新值。

## Send

动态路由原语，用于运行时决定将状态发送到哪个节点，并自定义传递的数据。
Send(node_name：目标节点的名称, input_data：传递给目标节点的数据（可以是 state 或自定义字典）)

## Command

通过一个或多个命令来更新图的状态并向节点发送消息，控制流程跳转，让节点不仅能更新数据，还能决定下一步执行哪个节点
Command(goto=下一步跳转的节点, update=哪些字段合并到全局, resume=用来恢复执行的值)
...
# 使用 thread_id 区分不同会话
config_user_a = {"configurable": {"thread_id": "xxx"}}
...
MemorySaver可以替换成SqliteSaver或PostgreSaver并连接到本地数据库。

## Human in the loop

```python
graph = graph_builder.compile( checkpointer=memory, interrupt_before=[节点名称], )
events = graph.stream( {"messages": [("user", user_input)]}, config, stream_mode="values" )
for event in events:
```

使用 interrupt 为 agent 添加人机交互断点，在需要时进行人工监督和干预。

interrupt() 主要用于 Web 应用/人机协作场景 ，需要配合外部 UI（如 Agent Inbox、自定义前端）来收集用户输入并通过 Command(resume=...) 恢复执行。如果需要在终端中实现真正的交互式输入，应该使用 input()。
...
性质判断
- 风格与规范检查：命名是否符合规范、缩进格式是否一致、注释是否完整。这类确定性任务，使用静态分析工具+小模型处理；
- 逻辑与正确性分析：边界条件有没有覆盖、循环是否存在死锁、返回值有没有被错误处理。需要理解代码意图，推理能力要求中等；
- 安全漏洞扫描：sql注入、越权访问、密钥硬编码、不安全的反序列化。这类问题有明确的漏洞模式，但需要结合上下文判断是否真实存在，误报率控制是挑战；
- 架构与设计评审：改动和现有模块的边界是否清晰、是否引入不必要的耦合、是否违反了团队的架构约定。需要理解整个代码库的上下文，最难自动化。

A: 四个 specialist agent 并行运行，一个 orchestrator 负责拆分任务、分类、汇总结果，一个 synthetizer 负责合并四路输出、去重、排优先级、生成最终 review 报告。

Q: orchestrator 把任务分发给四个 specialist 时，每个 specialist 拿到的上下文应该是什么？
A: orchestrator 在分发时做上下文裁剪，安全 agent 拿到 diff+文件类型+已知的高风险函数列表；架构 agent 拿到 diff+相关模块的接口定义+架构约定文档的摘要。每个 specialist 只拿它需要的部分。

## 第二层：生产故障排查

Q：上线两周后，安全 agent 的误报率高达 40% ，怎么处理？
A：排查路径：1、先搞清楚误报的分布，有些是静态工具检测逻辑问题，有些是上下文信息不够（获得 diff ，但没有拿到被修改函数的调用方、没有文件的安全注解等信息）2、建立人工标注的反馈闭环：记录误报 case，建立标注数据集，定期校准 agent 的输出。

## 第三层：权衡决策

Q：要求控制成本
A：1、按 pr 类型分级触发：例如只改了单元测试、注释、配置文件的 pr，不跑架构 agent。orchestrator 在分发任务之前，先做一次轻量的 pr 分类，只有涉及跨模块、接口变动的 pr 才触发架构 agent，大幅降低成本
2、架构 agent 使用更小的上下文窗口和更聚焦的提示词。架构约定文档的内容很多，可以使用低成本的向量检索或关键词检索工具，提取与本次改动相关的段落内容，再放入上下文中。

# 编码、审查、重构代码原则

[andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills/blob/main/skills/karpathy-guidelines/SKILL.md)

## 编码前先思考

不要妄下结论。不要掩饰困惑。坦诚地权衡利弊。

实施前：
- 明确陈述假设。如有疑问，直接提出。
- 如果存在多种解释，请全部提出——不要默默地做出选择。
- 如果存在更简单的方法，请提出来。必要时要坚持己见。
- 如果有不清楚的地方，停下来，指出疑惑的地方，并提问。
...
- 对于不可能出现的情况，不进行错误处理。
- 如果50行能完成的任务被写了200行，即刻重写。

询问自己：“一位资深工程师会认为这过于复杂吗？”如果答案为肯定，那就简化它。
...
对于多步骤任务，简要说明计划。明确的成功标准能让你独立循环迭代。而模糊的标准则需要不断澄清。

# Get Shit Done

[Get Shit Done（GSD）实战使用文档：从安装到交付 | DeathWhispers](https://deathwhispers.github.io/posts/2026/04/10/gsd-get-shit-done-usage-guide/)